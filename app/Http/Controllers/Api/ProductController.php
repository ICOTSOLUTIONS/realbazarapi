...
class ProductController extends Controller
{
    public function home($role = null)
    {
        ...
        if ($role == 'retailer') {
            ...
            $justForYouSlider = HomePageImage::orderBy('id', 'DESC')->where('is_just_for_you', true)->where('is_retailer', true)->take(1)->get();
            $justForYouProduct = HomePageImage::orderBy('id', 'DESC')->where('is_just_for_you', true)->where('is_retailer', true)->skip(1)->take(6)->get();
    ...
    public function seller_top_sales()
    {
        ...
        $seller_top_sales = User::whereHas('role', function ($query) {
            ...
            $query->selectRaw('seller_id, count(seller_id) as total_orders')
                ->groupBy('seller_id')->whereHas('user_orders', function ($query1) {
